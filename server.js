...
app.post('/verificar', async (req, res) => {
  ...
  const { data: cartao } = await supabase
    .from('cartoes')
    .select('aluno_id, alunos(nome, turma, ativo)')
    .eq('uid', uidUpper)
    .eq('ativo', true)
    .single();

  if (!cartao || !cartao.alunos?.ativo) {
  ...
  const { aluno_id, alunos: { nome: aluno_nome, turma: aluno_turma } } = cartao;
  ...
  if (count > 0) {
    await gravarRegistro({ uid: uidUpper, aluno_id, aluno_nome, aluno_turma, status: 'negado', motivo: 'Lanche já retirado hoje', origem });
    return res.json({ acesso: false, motivo: 'Lanche já retirado hoje.' });
  }

  await gravarRegistro({ uid: uidUpper, aluno_id, aluno_nome, aluno_turma, status: 'liberado', origem });
  return res.json({ acesso: true, aluno: aluno_nome, turma: aluno_turma });
});
...
app.get('/sync-cache', async (req, res) => {
  if (!DEVICE_KEY || req.headers['x-device-key'] !== DEVICE_KEY) {
    return res.status(401).json({ erro: 'Não autorizado.' });
  }
  ...
  const uids = (cartoes || [])
    .filter(c => c.alunos?.ativo)
    .map(c => ({ uid: c.uid, aluno_id: c.aluno_id, nome: c.alunos.nome, turma: c.alunos.turma }));

  res.json({ uids });
});

// ─── Último cartão desconhecido (best-effort em serverless) ───────────────────
